...
public class LogoutWorkFlow extends WorkFlow {
	...
	public LogoutWorkFlow(String openId,int expire){
		this.openId = openId;
		this.expire = expire;
		this.wfName = "LogoutWorkFlow";
	}
	
	public LogoutWorkFlow(String openId){
	...
	public String getPrompt() {
		...
		if(user != null){
			if(user.getLocation() == null){
				...
				//return "下班打卡，您目前的位置未知，请打开GPS或者到室外信号好的地方再试。或者请关闭当前对话框口然后再重试；如还不行请确认已经允许微信能够访问你的位置信息，具体设置可以参考手册或询问管理员。";
				return "下班打卡，您目前的位置未知。\n\n<a href=\""+Config.appUrl+"/loginlogoutNote.html\">立即点击查阅解决方法。</a>";
			}
			location = WXUtil.convertBDAxis(user.getLocation().getLatitude(), user.getLocation().getLongitude());
			return "下班打卡，您目前的位置: "+WXUtil.geocoderBMap(location)+", 确认请回复\"1\",取消请回复\"0\" ";
		}else{
	...
	public WorkFlow end(String res) {
		if("1".equals(res)){
			...
			util.trace(this, "User["+openId+"] Logout ok @"+loginTime);
			WXUser user = service.getWXUser(this.openId);
			// String location = "";
			...
			if(location.split(",").length>1) location = location.split(",")[1]+","+location.split(",")[0];
		   	String response = service.invokeCRMHttpGet(3000,2000,"login", "openid="+openId,"logintime="+loginTime,"location="+location,"loginmode=logout");
		   	...
		   	if(response != null && response.length()>0){
		   		...
		   		if("success".equals(result.getResult()) || "0".equals(result.getResult())){
		   			return new LogoutOkWorkFlow(this.openId);
		   		}else{
		   			return new SysFailedWorkFlow("logout", result.getResult());
		   		}
		   	}else{
		   		util.warn(this, "UserLogout failure for response is null.");
		   		return new SysFailedWorkFlow("logout", "请求超时，请稍候再试.");
		   	}
		}else{
			util.trace(this, "User["+this.openId+"] logout cancel.");
			return new LogoutCancelWorkFlow(this.openId);
		}
	}
